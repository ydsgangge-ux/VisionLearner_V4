#!/bin/bash
# ============================================================
#  VisionLearner macOS/Linux 一键安装向导 v4.2
# ============================================================

cd "$(dirname "$0")"

echo ""
echo "============================================================"
echo "          VisionLearner 一键安装向导 v4.2"
echo "============================================================"
echo ""

# 步骤 1: 检查 Python 环境
echo "[1/6] 检查 Python 环境..."

# macOS 自带 python3，优先尝试 python3，再尝试 python
if command -v python3 &>/dev/null; then
    PYTHON=python3
elif command -v python &>/dev/null; then
    PYTHON=python
else
    echo ""
    echo "  [FAIL] 未找到 Python，请先安装 Python 3.8+"
    echo ""
    echo "  macOS 安装方式（任选一种）："
    echo "    1. 官网下载: https://www.python.org/downloads/"
    echo "    2. Homebrew: brew install python3"
    echo ""
    echo "  推荐使用 Python 3.12 或更高版本"
    echo ""
    read -p "按回车键退出..."
    exit 1
fi

PY_VERSION=$($PYTHON --version 2>&1)
echo "  [OK] ${PY_VERSION} 已安装"

# 步骤 2: 升级 pip
echo ""
echo "[2/6] 升级 pip 包管理器..."
$PYTHON -m pip install --upgrade pip --quiet 2>/dev/null
echo "  [OK] pip 已升级到最新版本"

# 步骤 3: 安装核心依赖
echo ""
echo "[3/6] 安装项目依赖（这可能需要几分钟）..."

$PYTHON -m pip install requests flask flask-cors schedule python-dotenv networkx numpy openai chromadb matplotlib --quiet 2>/dev/null

if [ $? -ne 0 ]; then
    echo "  [FAIL] 依赖安装失败，尝试单独安装..."
    echo ""
    echo "  正在尝试逐个安装..."
    $PYTHON -m pip install requests --quiet 2>/dev/null
    $PYTHON -m pip install flask flask-cors --quiet 2>/dev/null
    $PYTHON -m pip install schedule --quiet 2>/dev/null
    $PYTHON -m pip install python-dotenv --quiet 2>/dev/null
    $PYTHON -m pip install networkx --quiet 2>/dev/null
    $PYTHON -m pip install numpy --quiet 2>/dev/null
    $PYTHON -m pip install matplotlib --quiet 2>/dev/null
    echo ""
fi

# 检查 Flask 是否安装成功
$PYTHON -c "import flask; import flask_cors" 2>/dev/null
if [ $? -ne 0 ]; then
    echo ""
    echo "  [FAIL] Flask 安装失败，请手动安装："
    echo "  [CMD] pip install flask flask-cors"
    read -p "按回车键退出..."
    exit 1
fi
echo "  [OK] 所有依赖安装成功"

# 步骤 4: 检查配置文件
echo ""
echo "[4/6] 检查配置文件..."

if [ ! -f ".env" ]; then
    if [ -f ".env.example" ]; then
        echo "  [TIP] 未找到 .env 文件，正在创建..."
        cp .env.example .env 2>/dev/null
        echo "  [OK] 已创建 .env 文件"
        echo ""
        echo "  [IMPORTANT] 请编辑 .env 文件，填入你的 API 密钥！"
        echo "  [HELP] 查看 API_KEY_SETUP.md 了解如何获取密钥"
        echo "  [CMD] 用以下命令编辑: nano .env 或 open -e .env"
    else
        echo "  [WARN] 未找到配置文件模板"
    fi
else
    echo "  [OK] .env 配置文件已存在"
fi

# 步骤 5: 检查知识库
echo ""
echo "[5/6] 检查知识库..."
if [ -d "learning_data/goals" ]; then
    echo "  [OK] 已发现学习数据"
else
    echo "  [TIP] 首次使用建议导入预置知识体系"
    echo "  [CMD] 运行以下命令导入："
    echo "     python3 import_knowledge_plan.py"
    echo "     python3 import_ai_knowledge.py"
    echo ""
fi

# 步骤 6: 验证安装
echo ""
echo "[6/6] 验证系统状态..."
$PYTHON -c "from web_server import create_app; print('[OK] Web 服务器模块正常')" 2>/dev/null
if [ $? -ne 0 ]; then
    echo "  [WARN] Web 服务器验证失败，但应该可以正常运行"
fi

# 安装完成
echo ""
echo "============================================================"
echo "                     安装完成！"
echo "============================================================"
echo ""
echo "使用方式："
echo ""
echo "  [WEB] Web 模式（推荐）"
echo "     - 双击 \"启动Web服务.command\""
echo "     - 或运行：$PYTHON main.py --web"
echo ""
echo "  [CLI] 命令行模式"
echo "     - 双击 \"启动命令行模式.command\""
echo "     - 或运行：$PYTHON main.py"
echo ""
echo "首次使用："
echo "  1. 编辑 .env 填入 API 密钥（参考 API_KEY_SETUP.md）"
echo "  2. 运行 $PYTHON import_knowledge_plan.py"
echo "  3. 运行 $PYTHON import_ai_knowledge.py"
echo "  4. 启动系统开始学习！"
echo ""
echo "============================================================"
echo ""

read -p "是否立即启动 Web 服务？(Y/N): " choice
if [[ "$choice" == "Y" || "$choice" == "y" ]]; then
    echo ""
    echo "正在启动 VisionLearner Web 服务..."
    echo ""
    $PYTHON main.py --web
else
    echo ""
    echo "再见！有问题请查看 README.md"
fi
